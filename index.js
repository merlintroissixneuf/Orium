...
const factionTapsQuery =             SELECT                  SUM(CASE WHEN faction = 'BULLS' THEN tap_count ELSE 0 END) as bull_taps,                 SUM(CASE WHEN faction = 'BEARS' THEN tap_count ELSE 0 END) as bear_taps             FROM match_players WHERE match_id = $1;        ;
...
const leaderboardQuery =                     SELECT u.username, mp.tap_count                     FROM match_players mp                     JOIN users u ON mp.user_id = u.id                     WHERE mp.match_id = $1                     ORDER BY mp.tap_count DESC                ;
...
try {
const query =             SELECT                  u.username,                  u.email,                  w.hype_token_balance,                  w.orium_shard_balance              FROM users u             JOIN wallets w ON u.id = w.user_id             WHERE u.id = $1;        ;
